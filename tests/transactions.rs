...
async fn test_withdraw() {
    let from = "0x87bC2200f5066DFc22e987DAb486b979Cd254F4B".parse().unwrap();
    let trx = Transaction::from_withdraw_no_cache(
        40,
        0,
        Checksum256::from_bytes(&hex_to_bytes("1203ee37cfc4130ea7bf4885f3cbbf1fe85a55ce64709fe7357dcdc453e0ba1f")).unwrap(),
        WithdrawAction {
            to: Name::new("steferretto"),
            quantity: Asset::new(37000000, Symbol::new("TLOS", 4)),
        }, from).await;

    assert_eq!(trx.hash().to_string(), "0x38f3f0600ea400119b34289d198c048623520be84f0e9f98941bf033a8e1c49c");
}