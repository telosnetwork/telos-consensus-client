...
pub async fn dump_storage(telos_rpc: &str, block_delta: u32) -> TelosEVMStateJSON {
    let api_client =
        APIClient::<DefaultProvider>::default_provider(telos_rpc.into(), Some(5)).unwrap();
    ...
    let config_row = api_client
        .v1_chain
        .get_table_rows::<ConfigRow>(query_params)
        .await
    ...
    while has_more {
        ...
        let account_rows = api_client
            .v1_chain
            .get_table_rows::<AccountRow>(query_params)
            .await;
...
async fn dump_account_storage(
    ...
    while has_more {
        ...
        let account_state_rows = api_client
            .v1_chain
            .get_table_rows::<AccountStateRow>(query_params)
            .await;
        if let Ok(account_state_rows) = account_state_rows {
            ...
            for account_state_row in account_state_rows.rows {
                ...
                let telos_value: U256 =
                    U256::from_be_slice(account_state_row.value.data.as_slice());
                println!(
                    "Storage account: {:?} with scope: {:?} and key: {:?}",
                    address,
                    scope.unwrap(),
                    key
                );
